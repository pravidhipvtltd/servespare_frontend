...
export const CashierDashboard: React.FC = () => {
  ...
  const addToCart = (item: InventoryItem) => {
    ...
    if (item.quantity <= item.minStockLevel) {
      alert(`⚠️ Low Stock Alert: ${item.name} is running low (${item.quantity} units). Please inform storekeeper!`);
    }
  };
  ...
  const completeSale = (total: number) => {
    ...
    if (paymentMethod === 'cash') {
      ...
      const allCashTxns: CashTransaction[] = getFromStorage('cashTransactions', []);
  ...
  const todayCashSales = todayBills.filter(b => b.paymentMethod === 'cash').reduce((sum, b) => sum + b.total, 0);
        ...
        {shiftStarted && (
          <div className="px-4 pb-2">
            <div className="bg-green-900/30 border border-green-700 rounded-xl p-3">
              ...
              <p className="text-green-300 text-xs mt-1">Start: NPR {shiftStartCash.toLocaleString()}</p>
            </div>
          </div>
        )}
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          ...
          {!shiftStarted && activePanel === 'pos' && (
            <div className="bg-yellow-100 border border-yellow-300 rounded-lg px-4 py-2 flex items-center space-x-2">
              ...
              <span className="text-yellow-800 font-semibold">Shift Not Started</span>
            </div>
          )}
        </header>
...
const DashboardView: React.FC<any> = ({ 
  bills, todayRevenue, todayCashSales, inventory, onNavigate, shiftStarted, currentUser 
}) => {
    ...
    <div className="space-y-6">
      ...
      {!shiftStarted && (
        <div className="bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-6">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <h3 className="text-yellow-900 font-bold text-lg mb-2">Start Your Shift</h3>
              <p className="text-yellow-800 mb-4">Please start your shift and enter opening cash before processing any sales.</p>
              <button
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="space-y-3">
            ...
            {lowStockItems.map((item: InventoryItem) => (
              <div key={item.id} className="flex items-center justify-between p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
                <div className="flex items-center space-x-3">
                  <TrendingDown className="w-5 h-5 text-yellow-600" />
                  <div>
                    <div className="text-gray-900 font-semibold">{item.name}</div>
                    <div className="text-yellow-600 text-sm">Low Stock: {item.quantity} units remaining</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <p className="text-orange-100 text-sm mb-4">Process customer purchase and create invoice</p>
        ...
        >
          ...
          <p className="text-blue-100 text-sm mb-4">View sales summary and cash reconciliation</p>
          <div className="flex items-center text-white font-semibold">
      ...
      {/* Recent Sales */}
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        <h3 className="text-gray-900 font-bold text-lg mb-4">Recent Sales (Your Transactions)</h3>
        <div className="space-y-3">
          {bills.slice(0, 5).map((bill: Bill) => (
            <div key={bill.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
              <div>
                ...
                <div className="text-gray-500 text-sm">{new Date(bill.createdAt).toLocaleTimeString()}</div>
              </div>
              <div className="text-right">
                <div className="text-gray-900 font-bold text-lg">NPR {bill.total.toLocaleString()}</div>
                <div className="text-gray-500 text-xs">{bill.paymentMethod.toUpperCase()}</div>
              </div>
            </div>
          ))}
        </div>
...
const POSView: React.FC<any> = ({
  inventory, cart, searchQuery, setSearchQuery,
  customerName, setCustomerName, customerPhone, setCustomerPhone,
  customerType, setCustomerType, paymentMethod, setPaymentMethod,
  discount, setDiscount, discountType, setDiscountType,
  taxRate, setTaxRate, addToCart, updateCartQuantity,
  removeFromCart, calculateTotals, handleCheckout,
  shiftStarted, showChangeCalculator, setShowChangeCalculator,
  amountReceived, setAmountReceived, completeSale
}) => {
          ...
          {inventory.map((item: InventoryItem) => (
            ...
            >
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-3 ${
                item.quantity === 0 ? 'bg-red-100' : item.quantity <= item.minStockLevel ? 'bg-yellow-100' : 'bg-orange-100'
              }`}>
                <Package className={`w-6 h-6 ${
                  item.quantity === 0 ? 'text-red-600' : item.quantity <= item.minStockLevel ? 'text-yellow-600' : 'text-orange-600'
                }`} />
              </div>
              <h4 className="text-gray-900 font-semibold text-sm mb-1 line-clamp-2">{item.name}</h4>
              <p className="text-orange-600 font-bold text-lg mb-2">NPR {(item.retailPrice || item.price).toLocaleString()}</p>
              <div className="flex items-center justify-between text-xs">
                <span className={`font-semibold ${
                  item.quantity === 0 ? 'text-red-600' : item.quantity <= item.minStockLevel ? 'text-yellow-600' : 'text-green-600'
                }`}>
            ...
            <div className="space-y-4 mb-6">
              ...
              {amountReceived > 0 && (
                <div className={`rounded-xl p-4 ${
                  ...
                  <div className={`font-bold text-3xl ${change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 className="text-gray-900 font-bold text-lg mb-4">Your Sales Today</h3>
        <div className="space-y-3">
          {bills.length === 0 ? (
            <div className="text-center text-gray-400 py-12">
            ...
            bills.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(bill => (
              <div key={bill.id} className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <div className="text-gray-900 font-bold">{bill.billNumber}</div>
                    <div className="text-gray-600 text-sm">{bill.customerName} • {bill.customerType}</div>
                    <div className="text-gray-500 text-xs">{new Date(bill.createdAt).toLocaleString()}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-gray-900 text-2xl font-bold">NPR {bill.total.toLocaleString()}</div>
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                      bill.paymentMethod === 'cash' ? 'bg-green-100 text-green-700' :
                      bill.paymentMethod === 'esewa' ? 'bg-blue-100 text-blue-700' :
                      bill.paymentMethod === 'khalti' ? 'bg-purple-100 text-purple-700' :
                ...
                <div className="pt-3 border-t border-gray-200">
                  <div className="text-gray-600 text-sm font-semibold mb-2">Items:</div>
                  {bill.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-sm text-gray-700 mb-1">
                      <span>{item.itemName} × {item.quantity}</span>
                      <span className="font-semibold">NPR {item.total.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
...
const CashDrawerView: React.FC<any> = ({
  cashTransactions, todayCashSales, shiftStartCash, shiftStarted, startShift, currentUser, onUpdate
}) => {
  ...
  const todayCashTransactions = cashTransactions.filter((txn: CashTransaction) =>
    ...
    <div className="space-y-6">
      {!shiftStarted ? (
        <div className="bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-yellow-900 font-bold text-xl mb-2">Start Your Shift</h3>
          <p className="text-yellow-800 mb-6">Please count and enter the opening cash in drawer</p>
          <button
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border-2 border-blue-200">
              ...
              <p className="text-blue-600 text-3xl font-bold">NPR {shiftStartCash.toLocaleString()}</p>
            </div>
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border-2 border-green-200">
              <ArrowUpCircle className="w-8 h-8 text-green-600 mb-3" />
              <h4 className="text-green-700 font-semibold mb-1">Cash In (Sales)</h4>
              <p className="text-green-600 text-3xl font-bold">NPR {todayCashSales.toLocaleString()}</p>
            </div>
            <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-2xl p-6 border-2 border-orange-200">
              <DollarSign className="w-8 h-8 text-orange-600 mb-3" />
              <h4 className="text-orange-700 font-semibold mb-1">Current in Drawer</h4>
              <p className="text-orange-600 text-3xl font-bold">NPR {currentCashInDrawer.toLocaleString()}</p>
            </div>
          </div>
          ...
          {/* Transactions List */}
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <h3 className="text-gray-900 font-bold text-lg mb-4">Today's Cash Transactions</h3>
            <div className="space-y-3">
              {todayCashTransactions.length === 0 ? (
                ...
                todayCashTransactions.sort((a: CashTransaction, b: CashTransaction) =>
                  ...
                  <div key={txn.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <div className="text-gray-500 text-sm">{new Date(txn.createdAt).toLocaleTimeString()}</div>
                      </div>
                    </div>
      ...
      {showStartShiftModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full">
            <h3 className="text-gray-900 font-bold text-xl mb-6">Start Your Shift</h3>
            <div className="mb-6">
              <label className="block text-gray-700 font-semibold mb-2">Opening Cash in Drawer</label>
              <input
                type="number"
                value={startingCash}
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 rounded-xl font-semibold text-white ${
                  txnType === 'in' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
                }`}
              >
...
const EndOfDayView: React.FC<any> = ({
  todayBills, todayRevenue, todayCashSales, cashTransactions, shiftStartCash, shiftStarted
}) => {
  ...
  const paymentBreakdown = [
    { method: 'Cash', amount: todayBills.filter(b => b.paymentMethod === 'cash').reduce((sum, b) => sum + b.total, 0) },
    { method: 'eSewa', amount: todayBills.filter(b => b.paymentMethod === 'esewa').reduce((sum, b) => sum + b.total, 0) },
    { method: 'Khalti', amount: todayBills.filter(b => b.paymentMethod === 'khalti').reduce((sum, b) => sum + b.total, 0) },
    { method: 'Card', amount: todayBills.filter(b => b.paymentMethod === 'card').reduce((sum, b) => sum + b.total, 0) },
    { method: 'Bank', amount: todayBills.filter(b => b.paymentMethod === 'bank').reduce((sum, b) => sum + b.total, 0) },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-8 text-white">
        ...
        <p className="text-blue-100">{new Date().toLocaleDateString('en-NP', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="text-gray-900 text-3xl font-bold">{todayBills.length}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h4 className="text-gray-600 mb-2">Total Revenue</h4>
          <p className="text-green-600 text-3xl font-bold">NPR {todayRevenue.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h4 className="text-gray-600 mb-2">Cash Sales</h4>
          <p className="text-orange-600 text-3xl font-bold">NPR {todayCashSales.toLocaleString()}</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {paymentBreakdown.map((item, idx) => (
            <div key={idx} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl">
              ...
              <span className="text-gray-900 font-bold text-lg">NPR {item.amount.toLocaleString()}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="flex justify-between p-4 bg-orange-50 border-2 border-orange-300 rounded-xl">
              ...
              <span className="text-orange-600 font-bold text-xl">NPR {expectedCash.toLocaleString()}</span>
            </div>
          </div>
          ...
          {actualCashCount > 0 && (
            <div className={`mt-6 p-6 rounded-xl border-2 ${
              ...
              <div className="flex justify-between items-center mb-2">
                <span className="text-gray-700 font-semibold">Actual Cash Count</span>
                <span className="text-gray-900 font-bold text-xl">NPR {actualCashCount.toLocaleString()}</span>
              </div>
              ...
              {variance !== 0 && (
                <div className={`mt-3 p-3 rounded-lg ${
                  ...
                  <p className={`text-sm ${
                    Math.abs(variance) <= 100 ? 'text-yellow-800' : 'text-red-800'
                  }`}>
      ...
      {showCountModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full">
            <h3 className="text-gray-900 font-bold text-xl mb-6">Count Cash in Drawer</h3>
            <div className="mb-6">
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-4">
                <div className="text-orange-700 text-sm mb-1">Expected Amount</div>
                <div className="text-orange-600 font-bold text-2xl">NPR {expectedCash.toLocaleString()}</div>
              </div>
              <label className="block text-gray-700 font-semibold mb-2">Actual Cash Count</label>
              <input
                type="number"
            ...
            <div className="flex space-x-3">
              ...
              <button
                onClick={() => {
                  ...
                  alert('✅ Cash count recorded. Report saved for manager review.');
                }}
