...
import { 
  Building, User, Mail, Phone, MapPin, Upload, FileText, 
  Image, CreditCard, ArrowLeft, ArrowRight, Check, AlertCircle, X
} from 'lucide-react';
...
export const BusinessRegistrationForm: React.FC<BusinessRegistrationFormProps> = ({ onBack, onSuccess }) => {
  ...
  const packages = {
    basic: { name: 'Basic', price: 5000, features: ['1 User', '100 Products', 'Basic Reports', 'Email Support'] },
    pro: { name: 'Professional', price: 15000, features: ['5 Users', '1000 Products', 'Advanced Reports', 'Priority Support', 'Multi-Branch'] },
    enterprise: { name: 'Enterprise', price: 35000, features: ['Unlimited Users', 'Unlimited Products', 'Custom Reports', '24/7 Support', 'API Access'] }
  };
  ...
  const handleFileChange = (field: keyof RegistrationData, file: File | null) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [panVatBase64, regCertBase64, ownerPhotoBase64, citizenshipPhotoBase64] = await Promise.all([
        formData.panVatCertificate ? fileToBase64(formData.panVatCertificate) : Promise.resolve(''),
        formData.registrationCertificate ? fileToBase64(formData.registrationCertificate) : Promise.resolve(''),
        formData.ownerPhoto ? fileToBase64(formData.ownerPhoto) : Promise.resolve(''),
        formData.citizenshipPhoto ? fileToBase64(formData.citizenshipPhoto) : Promise.resolve('')
      ]);
      ...
      const pendingRegistration = {
        ...
        packageInfo: {
          ...
          price: formData.selectedPackage ? packages[formData.selectedPackage].price : 0,
          features: formData.selectedPackage ? packages[formData.selectedPackage].features : []
        },
        paymentInfo: {
          ...
          paymentDate: formData.paymentStatus === 'completed' ? new Date().toISOString() : ''
        },
        documents: {
      ...
      // Debug logging
      console.log('✅ New registration saved:', pendingRegistration.businessInfo.name);
...
📋 Status: ${formData.paymentStatus === 'completed' ? 'Payment Verified - Pending Admin Approval' : 'Payment Pending'}
          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block mb-2 font-medium text-gray-700">PAN/VAT Number *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block mb-2 font-medium text-gray-700">Business Phone *</label>
                  <input
                    type="tel"
          ...
          {step === 2 && (
            ...
            >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block mb-2 font-medium text-gray-700">Citizenship Number *</label>
                  <input
                    type="text"
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${
                    formData.selectedPackage === 'basic' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'
                  }`}
                ...
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${
                    formData.selectedPackage === 'pro' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'
                  }`}
                ...
                <div
                  className={`p-4 border rounded-lg cursor-pointer ${
                    formData.selectedPackage === 'enterprise' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'
                  }`}
                  onClick={() => handleInputChange('selectedPackage', 'enterprise')}
                >
                  <h4 className="text-lg font-bold text-indigo-600">Enterprise</h4>
            ...
            >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <FileUploadField
                  ...
                  onChange={(file) => handleFileChange('panVatCertificate', file)}
                />
                ...
                <FileUploadField
                  ...
                  onChange={(file) => handleFileChange('citizenshipPhoto', file)}
                  accept="image/*"
                />
              </div>
          ...
          {step === 5 && (
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block mb-2 font-medium text-gray-700">Login Email</label>
                  <input
                    type="email"
                    value={formData.businessEmail}
                    disabled
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4">
                <p className="text-sm text-yellow-800">
                  <strong>Important:</strong> After submitting your registration, your account will be reviewed by our Super Admin. 
                  You will be notified via email once your account is approved and you can start using the system.
                </p>
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              {formData.selectedPackage && (
                <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-lg p-6">
                  <h4 className="font-semibold text-indigo-900 mb-3">Package Summary</h4>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-indigo-600">Selected Package</p>
                      <p className="text-lg font-bold text-indigo-900">{packages[formData.selectedPackage].name}</p>
                    </div>
                    <div>
                      <p className="text-sm text-indigo-600">Amount to Pay</p>
                      <p className="text-2xl font-bold text-indigo-900">NPR {packages[formData.selectedPackage].price.toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-indigo-200">
                    <p className="text-sm text-indigo-700 font-medium mb-2">Features:</p>
                    <ul className="grid md:grid-cols-2 gap-2 text-sm text-indigo-600">
                      {packages[formData.selectedPackage].features.map((feature, index) => (
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      localStorage.setItem('pending_registration_data', JSON.stringify({
                        ...
                        packagePrice: packages[formData.selectedPackage].price
                      }));
                      ...
                      try {
                        initiatePayment({
                          ...
                          productName: `Serve Spares ${packages[formData.selectedPackage].name} Package`,
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <h5 className="text-lg font-semibold text-gray-800">eSewa</h5>
                      <p className="text-xs text-gray-500 text-center">Pay with eSewa digital wallet</p>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setPaymentProcessing(false);
                        setError(error.message || 'Failed to initiate FonePay payment');
                      }
                    }}
                    disabled={paymentProcessing || !formData.selectedPackage}
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <h5 className="text-lg font-semibold text-gray-800">FonePay</h5>
                      <p className="text-xs text-gray-500 text-center">Pay with your bank account</p>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 pt-4">
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-yellow-800">For Testing Only</p>
                      <p className="text-xs text-yellow-700 mt-1">
                        Click the button below to simulate a successful payment without using real payment gateways.
