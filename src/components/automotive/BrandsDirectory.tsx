...
import { Plus, Search, Package, Calendar, TrendingUp, Star, Car, Bike, RefreshCw, BarChart3, Filter, Download, Edit2, Trash2, Eye, Settings } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import { getFromStorage, saveToStorage } from '../../utils/mockData';
import { Brand } from '../../types/automotive';
import { listenToEvent } from '../../utils/eventBus';

export const BrandsDirectory: React.FC<{ onSelectBrand: (brand: Brand) => void }> = ({ onSelectBrand }) => {
  const { currentUser } = useAuth();
  const [brands, setBrands] = useState<Brand[]>([]);
  const [vehicleTypeFilter, setVehicleTypeFilter] = useState<'two_wheeler' | 'four_wheeler'>('two_wheeler');
  const [searchQuery, setSearchQuery] = useState('');
  const [isAdding, setIsAdding] = useState(false);
  const [editingBrand, setEditingBrand] = useState<Brand | null>(null);
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [sortBy, setSortBy] = useState<'name' | 'parts' | 'vehicles' | 'updated'>('name');
  ...
  const loadBrands = () => {
    ...
    setBrands(allBrands.filter((b: Brand) => b.workspaceId === currentUser?.workspaceId));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', vehicleType: 'two_wheeler', logo: '', description: '', country: '', website: '', established: '' });
    setIsAdding(false);
  ...
  const handleDelete = (brandId: string) => {
    if (confirm('Are you sure you want to delete this brand? All associated vehicles and parts will remain but will need to be reassigned.')) {
      const allBrands = getFromStorage('automotive_brands', []);
      saveToStorage('automotive_brands', allBrands.filter((b: Brand) => b.id !== brandId));
      loadBrands();
    }
  };
  ...
  // Popular brands
  const popular2WBrands = ['Honda', 'Hero', 'Bajaj', 'TVS', 'Yamaha', 'Suzuki', 'Royal Enfield', 'KTM', 'Kawasaki', 'Ducati', 'Harley-Davidson', 'Aprilia'];
  const popular4WBrands = ['Maruti Suzuki', 'Hyundai', 'Tata', 'Mahindra', 'Kia', 'Toyota', 'Honda', 'Volkswagen', 'Ford', 'Nissan', 'Renault', 'Skoda', 'MG', 'Jeep', 'BMW', 'Mercedes-Benz', 'Audi'];

  const quickAddBrand = (brandName: string) => {
    const allBrands = getFromStorage('automotive_brands', []);
    
    const exists = allBrands.some((b: Brand) => 
      b.name === brandName && 
  ...
  const selectAllBrands = () => {
    const popularBrands = vehicleTypeFilter === 'two_wheeler' ? popular2WBrands : popular4WBrands;
    const allBrands = getFromStorage('automotive_brands', []);
    
    // Only select brands that don't already exist
    const availableBrands = popularBrands.filter(brandName => 
      !allBrands.some((b: Brand) => 
        b.name === brandName && 
        b.vehicleType === vehicleTypeFilter &&
        b.workspaceId === currentUser?.workspaceId
      )
  ...
  const exportBrands = () => {
    ...
    const csvData = filteredBrands.map(brand => ({
      ...
      'Vehicle Type': brand.vehicleType === 'two_wheeler' ? '2-Wheeler' : '4-Wheeler',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-3xl font-bold flex items-center space-x-3 mb-2">
              ...
              <span>Brands & Vehicles Directory</span>
            </h3>
          ...
          <button
            ...
            className="flex items-center space-x-2 px-8 py-4 bg-white text-blue-600 rounded-xl hover:bg-blue-50 transition-all shadow-lg transform hover:scale-105 font-bold"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm text-blue-100">2-Wheeler Brands</span>
            </div>
            <p className="text-3xl font-bold">{stats.total2W}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            <div className="flex items-center space-x-2 mb-2">
              <Car className="w-5 h-5" />
              <span className="text-sm text-blue-100">4-Wheeler Brands</span>
            </div>
            <p className="text-3xl font-bold">{stats.total4W}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            <div className="flex items-center space-x-2 mb-2">
              <Package className="w-5 h-5" />
              <span className="text-sm text-blue-100">Total Models</span>
            </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm text-blue-100">Total Parts</span>
            </div>
            ...
            <div className="bg-gray-100 rounded-xl p-2 inline-flex shadow-inner">
              ...
              >
                ...
                <span className={`px-2 py-1 rounded-full text-xs ${
                  vehicleTypeFilter === 'two_wheeler' ? 'bg-white/20' : 'bg-gray-300 text-gray-700'
                }`}>
                  {stats.total2W}
                </span>
              </button>
              ...
              >
                ...
                <span>4-Wheeler</span>
                <span className={`px-2 py-1 rounded-full text-xs ${
                  vehicleTypeFilter === 'four_wheeler' ? 'bg-white/20' : 'bg-gray-300 text-gray-700'
                }`}>
                  {stats.total4W}
                </span>
              </button>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-gray-900 font-bold text-lg">
                    Showing {filteredBrands.length} of {brands.filter(b => b.vehicleType === vehicleTypeFilter).length} brands
                  </p>
                  <p className="text-gray-600 text-sm">
                    {vehicleTypeFilter === 'two_wheeler' ? '🏍️ 2-Wheeler brands' : '🚗 4-Wheeler brands'}
                  </p>
                </div>
              </div>
            </div>
      ...
      {isAdding && (
        <div className="bg-white rounded-2xl border-2 border-blue-200 p-8 shadow-2xl">
          <h4 className="font-bold text-gray-900 text-2xl mb-6 flex items-center space-x-2">
            {editingBrand ? <Edit2 className="w-7 h-7 text-blue-600" /> : <Plus className="w-7 h-7 text-blue-600" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, vehicleType: e.target.value as 'two_wheeler' | 'four_wheeler' })}
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-lg"
                >
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border-2 border-blue-200 p-8">
          <div className="flex items-center justify-between mb-6">
            <h4 className="font-bold text-gray-900 text-xl flex items-center space-x-2">
              ...
              <span>Quick Add Popular {vehicleTypeFilter === 'two_wheeler' ? '2-Wheeler' : '4-Wheeler'} Brands</span>
            </h4>
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors text-sm font-bold"
              >
                Clear
              </button>
              <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {(vehicleTypeFilter === 'two_wheeler' ? popular2WBrands : popular4WBrands).map(brandName => {
              const isSelected = selectedBrands.includes(brandName);
              const allBrands = getFromStorage('automotive_brands', []);
              const alreadyExists = allBrands.some((b: Brand) => 
                b.name === brandName && 
                b.vehicleType === vehicleTypeFilter &&
                b.workspaceId === currentUser?.workspaceId
              );
              ...
              return (
                <div
                  key={brandName}
                  onClick={() => !alreadyExists && toggleBrandSelection(brandName)}
                ...
                >
                  ...
                  {alreadyExists && (
                    <div className="text-xs mt-1 text-gray-500">Already added</div>
                  )}
                </div>
              );
            })}
      ...
      {filteredBrands.length === 0 && searchQuery ? (
        <div className="text-center py-20 bg-white rounded-2xl border-2 border-gray-200">
          ...
          <p className="text-gray-500 text-lg">Try adjusting your search or add a new brand</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className={`p-6 ${
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="w-20 h-20 rounded-full bg-white flex items-center justify-center text-4xl font-bold shadow-lg"
                    style={{ color: brand.vehicleType === 'two_wheeler' ? '#f97316' : '#3b82f6' }}>
                    {brand.name.charAt(0)}
                  </div>
                  <span className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold text-white flex items-center space-x-1">
                    {brand.vehicleType === 'two_wheeler' ? <Bike className="w-3 h-3" /> : <Car className="w-3 h-3" />}
                    <span>{brand.vehicleType === 'two_wheeler' ? '2W' : '4W'}</span>
                  </span>
                </div>
                <h4 className="text-white font-bold text-2xl mb-1">{brand.name}</h4>
                {(brand as any).country && (
                  <p className="text-white/80 text-sm">📍 {(brand as any).country}</p>
                )}
              </div>
              ...
              <div className="p-6 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-blue-50 rounded-lg p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-gray-900">{brand.totalParts || 0}</p>
                    <p className="text-xs text-gray-600">Parts</p>
                  </div>
                  <div className="bg-purple-50 rounded-lg p-3 text-center">
                    <Car className="w-5 h-5 text-purple-600 mx-auto mb-1" />
                    <p className="text-2xl font-bold text-gray-900">{brand.totalVehicles || 0}</p>
                    <p className="text-xs text-gray-600">Models</p>
                  </div>
                </div>

                {(brand as any).description && (
                  <p className="text-gray-600 text-sm line-clamp-2">{(brand as any).description}</p>
                )}
