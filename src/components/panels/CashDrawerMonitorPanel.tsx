...
import { Clock, DollarSign, TrendingUp, TrendingDown, Minus, X, Eye, Flag, XCircle, FileText, AlertTriangle, CheckCircle, Calendar, User, Building2 } from 'lucide-react';
...
export const CashDrawerMonitorPanel: React.FC = () => {
  ...
  const [selectedShift, setSelectedShift] = useState<CashDrawerShift | null>(null);
  ...
  const loadShifts = () => {
    ...
    setShifts(allShifts.filter((s: CashDrawerShift) => s.workspaceId === currentUser?.workspaceId));
  };
  ...
  const stats = {
    ...
    closedShifts: filteredShifts.filter(s => s.status === 'closed' || s.status === 'force_closed').length,
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-900 text-2xl mt-1">{stats.closedShifts}</p>
            </div>
        ...
        <div className="bg-white rounded-lg border border-red-200 p-4 shadow-lg shadow-red-100/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-red-700 text-2xl mt-1">NPR {stats.totalLoss.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white rounded-lg border border-green-200 p-4 shadow-lg shadow-green-100/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-700 text-2xl mt-1">NPR {stats.totalSurplus.toLocaleString()}</p>
            </div>
            <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">
        ...
        <div className="bg-white rounded-lg border border-orange-200 p-4 shadow-lg shadow-orange-100/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-orange-700 text-2xl mt-1">{stats.flaggedShifts}</p>
            </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left text-gray-500 text-sm py-3 px-4">Cashier</th>
                <th className="text-left text-gray-500 text-sm py-3 px-4">Opened</th>
                <th className="text-left text-gray-500 text-sm py-3 px-4">Closed</th>
                <th className="text-left text-gray-500 text-sm py-3 px-4">Duration</th>
                <th className="text-right text-gray-500 text-sm py-3 px-4">Opening Amt</th>
                <th className="text-right text-gray-500 text-sm py-3 px-4">Closing Amt</th>
                <th className="text-right text-gray-500 text-sm py-3 px-4">Difference</th>
            ...
            <tbody>
              {filteredShifts.map((shift) => (
                <tr key={shift.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${getShiftRowClass(shift)}`}>
                  <td className="py-4 px-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="text-gray-900">{shift.cashierName}</div>
                        {shift.branchName && (
                  ...
                  <td className="py-4 px-4">
                    {shift.closedAt ? (
                      <>
                        <div className="text-gray-900 text-sm">
                          {new Date(shift.closedAt).toLocaleDateString('en-NP')}
                        </div>
                        <div className="text-xs text-gray-500">
                          {new Date(shift.closedAt).toLocaleTimeString('en-NP', { hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </>
                    ) : (
                  ...
                  <td className="py-4 px-4 text-right text-gray-900">
                    {shift.closingAmount !== undefined ? `NPR ${shift.closingAmount.toLocaleString()}` : '-'}
                  </td>
                  <td className="py-4 px-4 text-right">
                    {shift.difference !== undefined ? (
                      <div className="flex items-center justify-end space-x-1">
                        {shift.difference < 0 && <TrendingDown className="w-4 h-4 text-red-600" />}
                        {shift.difference > 0 && <TrendingUp className="w-4 h-4 text-green-600" />}
                        {shift.difference === 0 && <Minus className="w-4 h-4 text-gray-600" />}
                        <span className={getDifferenceColor(shift.difference)}>
                  ...
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-sm border ${STATUS_COLORS[shift.status]}`}>
              ...
              <div className={`rounded-lg p-6 mb-6 ${
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center space-x-2 mb-2">
                      <h4 className="text-gray-900 text-2xl">{selectedShift.cashierName}</h4>
                      {selectedShift.flagged && (
                        <Flag className="w-6 h-6 text-orange-600 animate-pulse" />
                      )}
                    </div>
                    {selectedShift.branchName && (
                      <p className="text-gray-600">{selectedShift.branchName}</p>
                    )}
                  </div>
                  <span className={`px-4 py-2 rounded-full text-sm border ${STATUS_COLORS[selectedShift.status]}`}>
                  ...
                  <div className="mt-4 p-4 bg-orange-100 rounded-lg border-2 border-orange-300">
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="flex-1">
                        <p className="text-orange-900 font-medium">Flagged for Suspicious Activity</p>
                        <p className="text-orange-800 text-sm mt-1">Reason: {selectedShift.flagReason}</p>
                        <p className="text-orange-700 text-xs mt-1">
                          Flagged by {selectedShift.flaggedBy} on {selectedShift.flaggedAt ? new Date(selectedShift.flaggedAt).toLocaleString('en-NP') : 'N/A'}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div>
                    ...
                    <p className="text-gray-900">{new Date(selectedShift.openedAt).toLocaleString('en-NP')}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-sm">Closed At</p>
                    <p className="text-gray-900">
                      {selectedShift.closedAt ? new Date(selectedShift.closedAt).toLocaleString('en-NP') : 'Still Open'}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-sm">Duration</p>
                    <p className="text-gray-900">{calculateDuration(selectedShift.openedAt, selectedShift.closedAt)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-sm">Transactions</p>
                    <p className="text-gray-900">{selectedShift.transactions.length}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="grid grid-cols-2 divide-x divide-gray-200">
                    <div className="p-4 bg-blue-50">
                      ...
                      <p className="text-blue-700 text-2xl">NPR {selectedShift.openingAmount.toLocaleString()}</p>
                    </div>
                    <div className="p-4 bg-purple-50">
                      <p className="text-gray-600 text-sm mb-1">Closing Amount</p>
                      <p className="text-purple-700 text-2xl">
                        {selectedShift.closingAmount !== undefined ? `NPR ${selectedShift.closingAmount.toLocaleString()}` : 'Not Closed'}
                      </p>
                    </div>
                  </div>
                  {selectedShift.difference !== undefined && (
                    <div className={`p-4 border-t ${
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          {selectedShift.difference < 0 && <TrendingDown className="w-5 h-5 text-red-600" />}
                          {selectedShift.difference > 0 && <TrendingUp className="w-5 h-5 text-green-600" />}
                          {selectedShift.difference === 0 && <Minus className="w-5 h-5 text-gray-600" />}
                          <span className="text-gray-700">
                            {selectedShift.difference < 0 ? 'Loss' : selectedShift.difference > 0 ? 'Surplus' : 'Balanced'}
                          </span>
                        </div>
                        <p className={`text-2xl ${getDifferenceColor(selectedShift.difference)}`}>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left text-gray-600 text-sm py-3 px-4">Time</th>
                        <th className="text-left text-gray-600 text-sm py-3 px-4">Type</th>
                        <th className="text-left text-gray-600 text-sm py-3 px-4">Description</th>
                        <th className="text-right text-gray-600 text-sm py-3 px-4">Amount</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              {selectedShift.notes && (
                <div className="mb-6">
                  ...
                  <div className="border border-gray-200 rounded-lg p-4">
                    <p className="text-gray-600 text-sm">{selectedShift.notes}</p>
                  </div>
                </div>
              )}
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-lg shadow-xl z-50 p-6">
            ...
            <div className="mb-4 p-4 bg-orange-50 rounded-lg border border-orange-200">
              ...
              <p className="text-gray-700 text-sm">
                <strong>Shift:</strong> {new Date(shiftToFlag.openedAt).toLocaleString('en-NP')}
              </p>
              {shiftToFlag.difference !== undefined && (
                <p className={`text-sm mt-2 ${getDifferenceColor(shiftToFlag.difference)}`}>
                  <strong>Difference:</strong> NPR {Math.abs(shiftToFlag.difference).toLocaleString()}
                  {shiftToFlag.difference < 0 ? ' (Loss)' : shiftToFlag.difference > 0 ? ' (Surplus)' : ' (Balanced)'}
                </p>
              )}
            </div>

            <div className="mb-4">
              <label className="block text-gray-700 text-sm mb-2">Reason for Flagging *</label>
              <textarea
                value={flagReason}
