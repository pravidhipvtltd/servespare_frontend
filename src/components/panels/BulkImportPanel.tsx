...
export const BulkImportPanel: React.FC = () => {
  ...
  const parseImportedData = (text: string) => {
    ...
    if (lines.length < 2) {
      popup.showError("File is empty or invalid", "Import Error");
      return;
    }
      ...
      for (const name of possibleNames) {
        const index = rawHeaders.findIndex((h) => {
          ...
          if (
            name.length >= 4 &&
            h.length >= 4 &&
            (h.includes(name.slice(0, 4)) || name.includes(h.slice(0, 4)))
          )
            return true;
          return false;
        });
    ...
    const columnMap = {
      ...
      bikeType: findColumnIndex([
        "biketype",
        "vtype",
        "vehicletype",
        "segment",
      ]),
      hsnCode: findColumnIndex([
        "hsncode",
        "hsn",
        "hscode",
        "hsnno",
      ...
      wholesalePrice: findColumnIndex([
        "wholesaleprice",
        "wholesale",
        "wp",
        "tradeprice",
      ...
      distributorPrice: findColumnIndex([
        "distributorprice",
        "distributor",
        "dp",
        "distprice",
      ...
      warrantyPeriod: findColumnIndex([
        "warrantyperiod",
        "warranty",
        "warr",
        "guaranteeperiod",
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const detectCategory = (value: string): string => {
        ...
        if (
          ...
          return "original";
        if (v.includes("local") || v.includes("aftermarket") || v === "l")
          return "local";
        return "local"; // Default to local
      };

      // Smart vehicle type detection (handles various formats)
      const detectVehicleType = (value: string): string => {
      ...
      const item: BulkItem = {
        ...
        wholesalePrice:
          parseNumber(getValue(columnMap.wholesalePrice)) || undefined,
        distributorPrice:
          parseNumber(getValue(columnMap.distributorPrice)) || undefined,
    ...
    console.log(
      `Successfully parsed ${items.length} items from ${lines.length - 1} rows`
    );

    if (items.length === 0) {
      popup.showError(
        "No valid items found in the file. Please check the format.",
        "Import Error"
      );
      return;
    }
  ...
  const handlePaste = () => {
    if (!pasteText.trim()) {
      popup.showError("Please paste your data first", "No Data");
      return;
    }
  ...
  const validateItems = (items: BulkItem[]) => {
    ...
    const duplicates = partNumbers.filter(
      (num, index) => partNumbers.indexOf(num) !== index
    );
    duplicates.forEach((num) => {
      ...
      errors.push({
        row: rowNum,
        field: "Part Number",
        message: `Duplicate part number: ${num}`,
      });
    });
  ...
  const handleQuickEntryImport = () => {
    const validItems = quickEntryRows.filter(
      (item) => item.name && item.partNumber && item.quantity > 0
    );
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // If there's an uploaded file, send it directly via FormData
      if (uploadedFile) {
        ...
        // Send bulk upload request to API with file
        response = await fetch(
          `${
            import.meta.env.VITE_API_BASE_URL
          }/stock-management/inventory/bulk_upload/`,
          {
            method: "POST",
            headers,
            body: formData,
        ...
        // Fallback: Transform bulk items to API format for non-file imports (paste/quick entry)
        const apiPayload = bulkItems.map((item) => ({
          item_name: item.name,
          ...
          min_stock_level: item.minStockLevel.toString(),
          price: item.price.toString(),
          mrp: item.mrp.toString(),
          distributor_price:
            item.distributorPrice?.toString() || item.price.toString(),
          wholesale_price:
            item.wholesalePrice?.toString() || item.price.toString(),
          retail_pricing: item.retailPrice?.toString() || item.mrp.toString(),
          storage_location: item.location || "Main Warehouse",
          warranty_period: item.warrantyPeriod || "no_warranty",
          barcode: item.barcode || item.partNumber,
          vehicle_bike_details: item.vehicleName || "",
          model: item.bikeModel || "",
          type: item.bikeType || "",
          party: item.partyName ? parseInt(item.partyName) : null,
          images: [],
        }));
        ...
        // Send bulk upload request to API
        response = await fetch(
          `${
            import.meta.env.VITE_API_BASE_URL
          }/stock-management/inventory/bulk_upload/`,
      ...
      if (response.ok) {
        ...
        const itemsToAdd: InventoryItem[] = bulkItems.map((item) => ({
        ...
        saveToStorage("inventory", [...existingInventory, ...itemsToAdd]);

        // Auto-extract categories and brands from the imported products
        if (currentUser?.workspaceId) {
          const extractedCategories = autoExtractCategories(
            itemsToAdd,
            currentUser.workspaceId
          );
          const extractedBrands = autoExtractBrands(
            itemsToAdd,
            currentUser.workspaceId
          );

          console.log(
            `Auto-extracted ${extractedCategories.length} new categories`
          );
          console.log(`Auto-extracted ${extractedBrands.length} new brands`);
        }

        // Emit import complete event for real-time UI updates
        emitImportComplete({
          totalRows: bulkItems.length,
          successCount: successCount,
          errorCount: bulkItems.length - successCount,
        });
        ...
        throw new Error(
          errorData.detail || errorData.message || "Failed to import items"
        );
      }
    } catch (error) {
      ...
      // Fallback to localStorage-only import if API fails
      const existingInventory = getFromStorage("inventory", []);
      const existingPartNumbers = existingInventory.map(
        (item: InventoryItem) => item.partNumber
      );
      ...
      // Auto-extract categories and brands
      if (currentUser?.workspaceId) {
        autoExtractCategories(itemsToAdd, currentUser.workspaceId);
        autoExtractBrands(itemsToAdd, currentUser.workspaceId);
      }
      ...
      popup.showError(
        `API import failed. Items saved locally: ${successCount}. ${
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Import multiple items at once using CSV, Excel, or quick entry
          </p>
        </div>
      </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-blue-700 text-sm mb-3">
                  Download our pre-formatted templates to ensure correct data
                  format
                </p>
            ...
            {importMethod === "paste" && (
              <div className="space-y-4">
                ...
                <p className="text-gray-600 text-sm">
                  Copy data from Excel/Sheets and paste below. First row should
                  be headers.
                </p>
                <textarea
                  value={pasteText}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    ...
                    <th className="px-3 py-3 text-left">Price (Rs)</th>
                    <th className="px-3 py-3 text-left">MRP (Rs)</th>
                    <th className="px-3 py-3 text-left">Retail (Rs)</th>
                    <th className="px-3 py-3 text-left">Wholesale (Rs)</th>
                    <th className="px-3 py-3 text-left">Distributor (Rs)</th>
                  ...
                  {bulkItems.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className="px-3 py-3 text-gray-600 max-w-[150px] truncate"
                          title={item.location}
                        >
                          {item.location || "-"}
                        </td>
