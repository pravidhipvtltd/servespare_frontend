...
export const PartiesPanel: React.FC = () => {
  ...
  const loadPartyTransactions = (partyId: string) => {
    ...
    setTransactions(
      allTransactions.filter((t: PartyTransaction) => t.partyId === partyId)
    );
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingParty) {
      const updated = allParties.map((p: Party) =>
        p.id === editingParty.id ? { ...p, ...formData } : p
      );
  ...
  const handleDelete = (partyId: string) => {
    popup.showConfirm(
      ...
      () => {
        ...
        setSelectedParties([]);
        loadParties();
        popup.showSuccess("Party deleted successfully!");
      },
      {
        type: "danger",
        details: [
          "This will also affect related inventory items",
  ...
  const handleBulkDelete = () => {
    if (selectedParties.length === 0) {
      popup.showError("Please select parties to delete", "No Selection");
      return;
    }

    popup.showConfirm(
      "Delete Multiple Parties",
      `Are you sure you want to delete ${selectedParties.length} part${
        selectedParties.length > 1 ? "ies" : "y"
      }?`,
      () => {
        const allParties = getFromStorage("parties", []);
        const filtered = allParties.filter(
          (p: Party) => !selectedParties.includes(p.id)
        );
        saveToStorage("parties", filtered);
        setSelectedParties([]);
        loadParties();
      ...
      },
      {
        ...
        details: [
          "This action cannot be undone",
          "Related inventory items will also be affected",
        ],
      }
    );
  };
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <select
                      ...
                      required
                    >
                      {Object.entries(PARTY_TYPE_LABELS).map(
                        ([value, label]) => (
              ...
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg p-6 mb-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4">
                    {viewingParty.logo ? (
                      <img
                        src={viewingParty.logo}
                        alt={viewingParty.name}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-3">
                        <span
                          className={`px-3 py-1 rounded-full text-sm ${
                            PARTY_TYPE_COLORS[viewingParty.type]
                          }`}
                        >
              ...
              <div className="mb-6">
                ...
                <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  <table className="w-full">
                    <tbody>
                      <tr className="border-b border-gray-200">
                        <td className="py-3 px-4 bg-gray-50 text-gray-600 w-1/3">
                          Party Name
                        </td>
                  ...
                  <table className="w-full">
                    <tbody>
                      <tr className="border-b border-gray-200">
                        ...
                        <td className="py-3 px-4 text-gray-900">
                          {PAYMENT_TERMS_LABELS[viewingParty.paymentTerms]}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-200">
                        <td className="py-3 px-4 bg-gray-50 text-gray-600 w-1/3">
                      ...
                      <tr className="border-b border-gray-200">
                        ...
                        <td className="py-3 px-4 text-blue-600">
                          NPR {viewingParty.openingBalance.toLocaleString()}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-3 px-4 bg-gray-50 text-gray-600 w-1/3">
                    ...
                    return items.length > 0 ? (
                      <table className="w-full">
                        <thead>
                          <tr className="bg-gray-50 border-b border-gray-200">
                            <th className="text-left py-3 px-4 text-gray-600 text-sm">
                              Item Name
                            </th>
                            <th className="text-left py-3 px-4 text-gray-600 text-sm">
                              Part Number
                            </th>
                            <th className="text-right py-3 px-4 text-gray-600 text-sm">
                              Quantity
                            </th>
                            <th className="text-right py-3 px-4 text-gray-600 text-sm">
                              Price
                      ...
                      <div className="p-8 text-center">
                        ...
                        <p className="text-gray-500 text-sm">
                          No inventory items linked to this party yet.
                        </p>
                      </div>
                    );
                  })()}
                      ...
                      <tbody>
                        {transactions.map((txn, idx) => (
                          ...
                          >
                            ...
                            >
                              {txn.type === "payment" ? "-" : "+"}NPR{" "}
                              {txn.amount.toLocaleString()}
                            </td>
                            <td className="py-3 px-4 text-gray-900 text-right">
                              NPR {(txn.balanceAfter || 0).toLocaleString()}
                            </td>
                          </tr>
                        ))}
                      </tbody>
