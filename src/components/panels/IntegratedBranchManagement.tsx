...
export const IntegratedBranchManagement: React.FC<{
  users: User[];
  bills: Bill[];
  onUpdate: () => void;
}> = ({ users, bills, onUpdate }) => {
  ...
  const handleAddVendor = (formData: any) => {
    ...
    const plan =
      SUBSCRIPTION_PLANS[
        formData.subscriptionPlan as keyof typeof SUBSCRIPTION_PLANS
      ];
  ...
  const handleAddUser = (formData: any) => {
    ...
    auditLog.unshift({
      ...
      userName: "Super Admin",
      action: "Create User",
      details: `Created user: ${formData.name} (${formData.role}) in ${selectedBranch.name}`,
      category: "user",
    });
    saveToStorage("auditLog", auditLog);
  ...
  const handleDeleteBranch = (branchId: string) => {
    ...
    const updatedVendors = allVendors.filter(
      (v: Vendor) => v.parentBranchId !== branchId
    );
  ...
  const handleBulkDelete = () => {
    ...
    if (
      !confirm(
        `⚠️ Delete ${selectedBranches.length} branch(es) and all associated vendors?\\n\\nThis action cannot be undone!`
      )
    )
      return;
    ...
    const updatedWorkspaces = allWorkspaces.filter(
      (w: Workspace) => !selectedBranches.includes(w.id)
    );
    saveToStorage("workspaces", updatedWorkspaces);

    // Delete associated vendors for all selected branches
    const updatedVendors = allVendors.filter(
      (v: Vendor) => !selectedBranches.includes(v.parentBranchId)
    );
    saveToStorage("vendors", updatedVendors);
  ...
  const getStats = () => {
    ...
    const totalUsers = users.filter((u) =>
      workspaces.some((w) => w.id === u.workspaceId)
    ).length;
  ...
  const filteredBranches = workspaces.filter(
    (branch) =>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="bg-gradient-to-br from-pink-500 to-purple-600 rounded-xl p-6 text-white">
          ...
          <p className="text-3xl font-bold">
            NPR {stats.monthlyFees.toLocaleString()}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginatedBranches.map((branch) => {
          ...
          const branchVendors = vendors.filter(
            (v) => v.parentBranchId === branch.id
          );
          const branchRevenue = branchBills.reduce(
            (sum, b) => sum + b.total,
            0
          );
          const vendorRevenue = branchVendors.reduce(
            (sum, v) => sum + v.monthlyFee,
            0
          );
          ...
          return (
            <div
              key={branch.id}
              className="bg-white rounded-xl border-2 border-gray-200 hover:border-purple-300 hover:shadow-xl transition-all"
            >
              <div className="p-6">
                {/* Header */}
                  ...
                  <div className="mb-4 p-3 bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg border border-pink-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-pink-900">
                        NPR {vendorRevenue.toLocaleString()}
                      </span>
                    </div>
                  </div>
                )}
      ...
      {showBranchDetails && selectedBranch && (
        <BranchDetailsModal
          ...
          vendors={vendors.filter(
            (v) => v.parentBranchId === selectedBranch.id
          )}
      ...
      {/* Add User Modal */}
      {showAddUserModal && selectedBranch && (
        <AddUserModal
          branchName={selectedBranch.name}
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-gray-600 mb-6">
          Adding to:{" "}
          <span className="font-semibold text-purple-600">{branchName}</span>
        </p>
            ...
            <div className="grid grid-cols-4 gap-3">
              {Object.entries(SUBSCRIPTION_PLANS).map(([key, plan]) => (
                <button
                  key={key}
                  onClick={() =>
                    setFormData({ ...formData, subscriptionPlan: key as any })
                  }
...
const AddUserModal: React.FC<{
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    password: "",
    role: "inventory_manager" as "admin" | "inventory_manager" | "cashier",
  });
  ...
  const roles = [
    ...
    },
    {
      value: "cashier",
      label: "Cashier",
      description: "Process sales and billing",
    },
  ];
...
const BranchDetailsModal: React.FC<{
  ...
  const monthlyFees = vendors.reduce((sum, v) => sum + v.monthlyFee, 0);
  const branchRoles =
    getFromStorage("branchRoles", {})[branch.id] || DEFAULT_ROLES;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Complete branch management and control
              </p>
            </div>
            <button
              onClick={onClose}
              ...
              {monthlyFees > 0 && (
                <div className="bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200 rounded-xl p-6">
                  ...
                  <p className="text-4xl font-bold text-pink-900">
                    NPR {monthlyFees.toLocaleString()}
                  </p>
                  <p className="text-pink-700 mt-2">
                    Per month from {vendors.length} vendor organizations
                  </p>
                </div>
              )}
            </div>
              ...
              {vendors.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                  ...
                  <p className="text-gray-600 font-semibold mb-2">
                    No Vendor Organizations
                  </p>
                  <p className="text-gray-500 text-sm mb-4">
                    Add vendor organizations to this branch
                  </p>
                  <button
                    onClick={onAddVendor}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {vendors.map((vendor) => {
                    ...
                    return (
                      <div
                        key={vendor.id}
                        className="bg-white border-2 border-gray-200 rounded-xl p-5 hover:border-orange-300 hover:shadow-lg transition-all"
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center space-x-3">
                            <div
                              className={`w-12 h-12 bg-gradient-to-br from-${plan.color}-500 to-${plan.color}-600 rounded-lg flex items-center justify-center`}
                            >
                        ...
                        <div
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold mb-4 bg-${plan.color}-100 text-${plan.color}-700`}
                        >
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between p-2 bg-gray-50 rounded">
                            ...
                            <span className="font-bold text-gray-900">
                              NPR {vendor.monthlyFee.toLocaleString()}
                            </span>
                          </div>
                          <div className="flex justify-between p-2 bg-gray-50 rounded">
                            <span className="text-sm text-gray-600">
                          ...
                          <div className="flex justify-between p-2 bg-green-50 rounded">
                            ...
                            <span className="font-bold text-green-900">
                              NPR {vendor.totalRevenue.toLocaleString()}
                            </span>
                          </div>
                        </div>
          ...
          {activeTab === "roles" && (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-sm text-gray-600">
                  Manage permissions for this branch
                </span>
              </div>
              ...
              {users.length === 0 ? (
                ...
                <div className="space-y-3">
                  {users.map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-blue-300 hover:shadow-md transition-all"
                    >
      ...
      <div className="bg-white rounded-2xl p-8 max-w-md w-full">
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100">
            ...
            <input
              ...
              onChange={(e) =>
                setEditedRole({
                  ...editedRole,
                  canAccessVendors: e.target.checked,
                })
              }
          ...
          <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100">
            ...
            <input
              type="checkbox"
              checked={editedRole.canManageBilling}
              onChange={(e) =>
                setEditedRole({
                  ...editedRole,
