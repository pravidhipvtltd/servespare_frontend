...
export const UserRolesPanel: React.FC = () => {
  ...
  const loadUsers = () => {
    ...
    setUsers(
      allUsers.filter((u: User) => u.workspaceId === currentUser?.workspaceId),
    );
  };
  ...
  const handleBulkDelete = () => {
    ...
    if (
      confirm(
        `⚠️ Are you sure you want to delete ${selectedUsers.length} user(s)?\\n\\nThis action cannot be undone!`,
      )
    ) {
      const filtered = allUsers.filter(
        (u: User) => !selectedUsers.includes(u.id),
      );
      saveToStorage("users", filtered);
      setSelectedUsers([]);
      loadUsers();
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-orange-800 text-sm mb-3">
                <strong>CRITICAL SECURITY NOTICE:</strong> The SuperAdmin role
                has{" "}
                <strong>
                  default status settings that CANNOT be changed by ANY user
                </strong>
                , including other admins. SuperAdmin accounts displayed below
                are permanently protected and <strong>CANNOT</strong> be
                modified by <strong>ANYONE</strong>.
              </p>
              <div className="bg-white/60 rounded-lg p-3 space-y-2 text-sm">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <span className="text-gray-700">
                      {" "}
                      SuperAdmin is ALWAYS ACTIVE in default state and cannot be
                      made inactive
                    </span>
                  </div>
                </div>
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <span className="text-gray-700">
                      {" "}
                      SuperAdmin accounts must always exist in the system
                    </span>
                  </div>
                </div>
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <span className="text-gray-700">
                      {" "}
                      SuperAdmin role is permanent and cannot be modified
                    </span>
                  </div>
                </div>
                <div className="flex items-start space-x-2 mt-3 pt-3 border-t border-orange-200">
                  ...
                  <div>
                    ...
                    <span className="text-green-800">
                      {" "}
                      These restrictions ensure system access, recovery
                      capabilities, and critical security functions are always
                      maintained.
                    </span>
                  </div>
                </div>
                ...
                >
                  ...
                  {user.avatar ? (
                    <img
                      src={user.avatar}
                      alt={user.name}
                      className="w-24 h-24 rounded-full object-cover shadow-lg transform group-hover:scale-110 transition-transform duration-300"
                    />
                  ) : (
                    <div
                      className={`w-24 h-24 rounded-full ${getRandomGradient(index)} flex items-center justify-center text-white text-2xl font-bold shadow-lg transform group-hover:scale-110 transition-transform duration-300`}
                    >
                ...
                <div className="p-5">
                  <div className="text-center mb-4">
                    ...
                    <div className="flex flex-col items-center space-y-1">
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${ROLE_COLORS[user.role]?.bg || "bg-gray-50"} ${ROLE_COLORS[user.role]?.text || "text-gray-700"}`}
                      >
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center space-x-3">
                          <div
                            className={`w-12 h-12 rounded-full ${getRandomGradient(index)} flex items-center justify-center text-white font-semibold ${
                              user.role === "super_admin"
                      ...
                      <td className="py-4 px-6">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${ROLE_COLORS[user.role].bg} ${ROLE_COLORS[user.role].text}`}
                        >
                        ...
                        {user.role === "super_admin" ? (
                          <div className="flex flex-col space-y-1">
                            ...
                            <span className="text-xs text-gray-500 italic pl-3">
                              Default State - Cannot be Inactive
                            </span>
                          </div>
                        ) : (
                          <button
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-orange-800 text-xs mb-2">
                        <strong>CRITICAL:</strong> The SuperAdmin role has{" "}
                        <strong>
                          default status settings that CANNOT be changed by ANY
                          user
                        </strong>
                      ...
                      <div className="bg-white/60 rounded-lg p-2 space-y-1.5 text-xs">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-gray-900">
                            <strong>Role:</strong> Locked to SuperAdmin (cannot
                            be changed)
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-gray-900">
                            <strong>Status:</strong> ALWAYS ACTIVE in default
                            state (cannot be made inactive)
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-green-900">
                            <strong>Editable:</strong> Name, Email, Phone,
                            Password, Avatar
                          </span>
                        </div>
                      </div>
          ...
          <div className="fixed right-0 top-0 h-full w-full md:w-[600px] bg-white shadow-2xl z-50 overflow-y-auto transform transition-transform duration-300">
            <div className="p-8">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-4">
                  <div
                    className={`w-14 h-14 rounded-full ${getRandomGradient(0)} flex items-center justify-center text-white font-bold text-xl`}
                  >
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-1 ${ROLE_COLORS[selectedUserForPermissions.role].bg} ${ROLE_COLORS[selectedUserForPermissions.role].text}`}
                    >
              ...
              <div className="space-y-6">
                ...
                <div className="border border-gray-200 rounded-2xl p-6 bg-gradient-to-br from-blue-50 to-white">
                  ...
                  <div className="space-y-3">
                    {Object.entries(permissions.inventoryAccess).map(
                      ([key, value]) => (
                        <label
                          key={key}
                        ...
                        >
                          ...
                          <div className="relative">
                            ...
                            />
                            <div
                              className={`w-11 h-6 rounded-full transition-colors ${value ? "bg-blue-600" : "bg-gray-300"}`}
                            >
                              <div
                                className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${value ? "translate-x-6" : "translate-x-0.5"} mt-0.5`}
                              ></div>
                            </div>
                          </div>
                        </label>
                ...
                <div className="border border-gray-200 rounded-2xl p-6 bg-gradient-to-br from-orange-50 to-white">
                  ...
                  <div className="space-y-3">
                    {Object.entries(permissions.financialAccess).map(
                      ([key, value]) => (
                        <label
                          key={key}
                          className="flex items-center justify-between p-3 bg-white rounded-xl hover:bg-gray-50 transition-colors cursor-pointer"
                        >
                          ...
                          <div className="relative">
                            ...
                            />
                            <div
                              className={`w-11 h-6 rounded-full transition-colors ${value ? "bg-orange-600" : "bg-gray-300"}`}
                            >
                              <div
                                className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${value ? "translate-x-6" : "translate-x-0.5"} mt-0.5`}
                              ></div>
                ...
                <div className="border border-gray-200 rounded-2xl p-6 bg-gradient-to-br from-green-50 to-white">
                  ...
                  <div className="space-y-3">
                    {Object.entries(permissions.priceEditing).map(
                      ([key, value]) => (
                        <label
                          key={key}
                          className="flex items-center justify-between p-3 bg-white rounded-xl hover:bg-gray-50 transition-colors cursor-pointer"
                        >
                          ...
                          <div className="relative">
                            ...
                            />
                            <div
                              className={`w-11 h-6 rounded-full transition-colors ${value ? "bg-green-600" : "bg-gray-300"}`}
                            >
                              <div
                                className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${value ? "translate-x-6" : "translate-x-0.5"} mt-0.5`}
                              ></div>
                ...
                <div className="border border-gray-200 rounded-2xl p-6 bg-gradient-to-br from-purple-50 to-white">
                  ...
                  <div className="space-y-3">
                    {Object.entries(permissions.supplierManagement).map(
                      ([key, value]) => (
                        <label
                          key={key}
                          className="flex items-center justify-between p-3 bg-white rounded-xl hover:bg-gray-50 transition-colors cursor-pointer"
                        >
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={() =>
                                setPermissions({
                                  ...permissions,
                                  supplierManagement: {
                                    ...permissions.supplierManagement,
                                    [key]: !value,
                                  },
                                })
                              }
                            ...
                            />
                            <div
                              className={`w-11 h-6 rounded-full transition-colors ${value ? "bg-purple-600" : "bg-gray-300"}`}
                            >
                              <div
                                className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${value ? "translate-x-6" : "translate-x-0.5"} mt-0.5`}
                              ></div>
