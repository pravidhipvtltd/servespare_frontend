...
import { 
  DollarSign, TrendingDown, Package, AlertTriangle, ShoppingCart, 
  CreditCard, Wallet, FileText, TrendingUp, Calendar, Clock,
  ShoppingBag, Users, Banknote, Receipt, ChevronDown, ChevronUp
} from 'lucide-react';
import { getFromStorage } from '../../utils/mockData';
import { useAuth } from '../../contexts/AuthContext';
import { Bill, CustomerOrder, InventoryItem, CashTransaction } from '../../types';
...
export const DayBookPanel: React.FC = () => {
  ...
  const loadData = () => {
    ...
    setCashTransactions(allCashTransactions.filter((ct: CashTransaction) => ct.workspaceId === currentUser?.workspaceId));
  };
  ...
  const todayInventory = inventory.filter(i => i.lastRestocked && isToday(i.lastRestocked));
  const todayCashIn = cashTransactions.filter(ct => ct.type === 'in' && isToday(ct.createdAt));
  const todayCashOut = cashTransactions.filter(ct => ct.type === 'out' && isToday(ct.createdAt));

  // Calculate metrics
  const totalIncome = todayBills.reduce((sum, b) => sum + b.total, 0) + 
                     todayOrders.filter(o => o.status === 'completed').reduce((sum, o) => sum + o.total, 0);
  
  const totalExpense = todayCashOut.reduce((sum, ct) => sum + ct.amount, 0);
  
  const totalProductEntry = todayInventory.length;
  
  const lowStockProducts = inventory.filter(i => i.quantity <= i.minStockLevel);
  
  const totalOrdersToday = todayBills.length + todayOrders.length;
  
  const totalItemsToday = todayBills.reduce((sum, b) => sum + b.items.reduce((s, i) => s + i.quantity, 0), 0) +
                         todayOrders.reduce((sum, o) => sum + o.items.reduce((s, i) => s + i.quantity, 0), 0);
  
  const cashOrders = todayBills.filter(b => b.paymentMethod === 'cash').length +
                    todayOrders.filter(o => o.paymentMethod === 'cod' || o.paymentMethod === 'paid').length;
  
  const creditOrders = todayBills.filter(b => b.paymentMethod === 'credit').length +
                      todayOrders.filter(o => o.paymentMethod === 'credit').length;
  
  const cardOrders = todayBills.filter(b => b.paymentMethod === 'card').length;
  const esewaOrders = todayBills.filter(b => b.paymentMethod === 'esewa').length;
  const fonepayOrders = todayBills.filter(b => b.paymentMethod === 'fonepay').length;
  
  const totalCashReceived = todayBills.filter(b => b.paymentMethod === 'cash').reduce((sum, b) => sum + b.total, 0);
  const totalCardReceived = todayBills.filter(b => b.paymentMethod === 'card').reduce((sum, b) => sum + b.total, 0);
  const totalEsewaReceived = todayBills.filter(b => b.paymentMethod === 'esewa').reduce((sum, b) => sum + b.total, 0);
  const totalFonepayReceived = todayBills.filter(b => b.paymentMethod === 'fonepay').reduce((sum, b) => sum + b.total, 0);
  const totalCreditAmount = todayBills.filter(b => b.paymentMethod === 'credit').reduce((sum, b) => sum + b.total, 0);
  ...
  todayBills.forEach(bill => {
    bill.items.forEach(item => {
      if (!itemSales[item.itemId]) {
        itemSales[item.itemId] = { name: item.itemName, quantity: 0, revenue: 0 };
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Daily business activity and metrics</p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-100 text-xs">
                {todayBills.length + todayOrders.filter(o => o.status === 'completed').length} transactions
              </p>
            </div>
        ...
        <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-5 text-white transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl mb-1">₹{totalExpense.toLocaleString()}</p>
              <p className="text-red-100 text-xs">{todayCashOut.length} expense entries</p>
            </div>
            <div className="w-14 h-14 bg-white bg-opacity-20 rounded-lg flex items-center justify-center">
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl mb-1">₹{netProfit.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-5 text-white transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-100 text-xs">{totalItemsToday} items sold</p>
            </div>
            <div className="w-14 h-14 bg-white bg-opacity-20 rounded-lg flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-2xl text-orange-600">{lowStockProducts.length}</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg transition-all duration-200">
          ...
          <p className="text-green-600 text-xs mt-1">₹{totalCashReceived.toLocaleString()}</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg transition-all duration-200">
          ...
          <p className="text-purple-600 text-xs mt-1">₹{totalCreditAmount.toLocaleString()}</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-green-700 text-sm">{cashOrders} orders</span>
                </div>
                <p className="text-gray-600 text-sm mb-1">Cash</p>
                <p className="text-green-900 text-xl">₹{totalCashReceived.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-blue-700 text-sm">{cardOrders} orders</span>
                </div>
                <p className="text-gray-600 text-sm mb-1">Card</p>
                <p className="text-blue-900 text-xl">₹{totalCardReceived.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-purple-50 rounded-lg p-4 border-l-4 border-purple-500">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-purple-700 text-sm">{esewaOrders} orders</span>
                </div>
                <p className="text-gray-600 text-sm mb-1">Esewa</p>
                <p className="text-purple-900 text-xl">₹{totalEsewaReceived.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-orange-50 rounded-lg p-4 border-l-4 border-orange-500">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-orange-700 text-sm">{fonepayOrders} orders</span>
                </div>
                <p className="text-gray-600 text-sm mb-1">Fonepay</p>
                <p className="text-orange-900 text-xl">₹{totalFonepayReceived.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-orange-50 rounded-lg p-4 border-l-4 border-orange-500">
                <div className="flex items-center justify-between mb-2">
                  <FileText className="w-5 h-5 text-orange-600" />
                  <span className="text-orange-700 text-sm">{creditOrders} orders</span>
                </div>
                <p className="text-gray-600 text-sm mb-1">Credit</p>
                <p className="text-orange-900 text-xl">₹{totalCreditAmount.toLocaleString()}</p>
              </div>
            </div>
          </div>
        )}
                  ...
                  <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-sm">{item.quantity} units sold</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-green-600">₹{item.revenue.toLocaleString()}</p>
        ...
        {expandedSection === 'timeline' && (
          <div className="p-6">
            {todayBills.length === 0 && todayOrders.length === 0 ? (
              ...
              <div className="space-y-3">
                {/* Bills */}
                {todayBills.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((bill) => (
                  <div key={bill.id} className="flex items-start space-x-4 p-4 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-gray-600 text-sm">{bill.customerName}</p>
                        </div>
                        <div className="text-right">
                ...
                {todayOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((order) => (
                  <div key={order.id} className="flex items-start space-x-4 p-4 bg-purple-50 rounded-lg border-l-4 border-purple-500">
                ...
                {lowStockProducts.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-gray-500 text-xs">Current / Min Stock</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  ₹{totalOrdersToday > 0 ? Math.round(totalIncome / totalOrdersToday).toLocaleString() : 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-orange-600">{lowStockProducts.length}</span>
              </div>
              <div className="flex justify-between">
