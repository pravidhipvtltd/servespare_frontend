...
export const EnhancedDashboardPanel: React.FC = () => {
  ...
  const [inventoryVelocities, setInventoryVelocities] = useState<
    Map<string, InventoryVelocity>
  >(new Map());
  ...
  const loadDashboardData = () => {
    ...
    setTopSellingCount(
      Array.from(velocities.values()).filter((v) => v.velocity === "fast")
        .length
    );
    setDeadStockCount(
      Array.from(velocities.values()).filter(
        (v) => v.velocity === "slow" && v.salesPerDay === 0
      ).length
    );
    setHighValueCount(
      inventory.filter((i) => i.quantity * i.price > 50000).length
    );
  ...
  const calculateInventoryVelocities = (
    ...
    inventory.forEach((item) => {
      ...
      const daysUntilStockout =
        salesPerDay > 0 ? Math.floor(item.quantity / salesPerDay) : 999;
      ...
      velocities.set(item.id, {
        ...
        velocityScore,
        daysUntilStockout:
          daysUntilStockout < 999 ? daysUntilStockout : undefined,
        salesPerDay,
      });
    });
  ...
  const generateRealisticAIInsights = (
    ...
    velocities.forEach((velocity, itemId) => {
      ...
      if (
        item &&
        velocity.daysUntilStockout &&
        velocity.daysUntilStockout <= 7
      ) {
        insights.push({
          id: `stockout-${itemId}`,
          ...
          icon: AlertTriangle,
          color:
            velocity.daysUntilStockout <= 3
              ? "text-red-600"
              : "text-yellow-600",
          bgColor:
            velocity.daysUntilStockout <= 3 ? "bg-red-50" : "bg-yellow-50",
        });
      }
    });
    ...
    if (lastWeekRevenue > 0) {
      ...
      if (Math.abs(cashflowChange) > 5) {
        insights.push({
          ...
          category: "cashflow",
          message: `Your branch's cashflow is trending ${
            cashflowChange > 0 ? "+" : ""
          }${cashflowChange.toFixed(1)}% this week.`,
          metric: `Rs${thisWeekRevenue.toLocaleString()}`,
    ...
    fastMovers.forEach(([itemId, velocity]) => {
      ...
      if (item) {
        insights.push({
          ...
          message: `${
            item.name
          } is trending up with ${velocity.salesPerDay.toFixed(
            1
          )} sales/day. Stock up now.`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 border-2 border-indigo-200 rounded-2xl p-6 shadow-lg">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">
              Real-time analysis of your business patterns
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {aiInsights.map((insight) => {
            ...
            return (
              <div
                key={insight.id}
                className={`${insight.bgColor} border-2 ${insight.bgColor
                  .replace("bg-", "border-")
                  .replace(
                    "-50",
                    "-200"
              ...
              >
                <div className="flex items-start space-x-3">
                  <div
                    className={`w-10 h-10 ${insight.bgColor.replace(
                      "-50",
                      "-200"
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-xs uppercase tracking-wider font-semibold mb-1 ${insight.color}`}
                    >
                      {insight.type}
                    </div>
                    ...
                    {insight.metric && (
                      <div
                        className={`inline-flex items-center px-2 py-1 ${insight.bgColor.replace(
                          "-50",
                          "-200"
                        )} rounded-lg text-xs font-semibold ${insight.color}`}
                      >
                        {insight.metric}
                      </div>
                    )}
        ...
        {aiInsights.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>
              No critical insights at the moment. Your business is running
              smoothly!
            </p>
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Visual representation of stock movement speed
            </p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-2" onMouseMove={handleMouseMove}>
          {Array.from(inventoryVelocities.values())
            .slice(0, 48)
            .map((velocity, index) => (
              <div
                key={velocity.id}
                className={`aspect-square ${getVelocityColor(
                  velocity.velocity
                )} rounded-lg cursor-pointer hover:scale-110 hover:shadow-lg transition-all hover:z-10 relative`}
                onMouseEnter={() =>
                  setHoveredCell({
