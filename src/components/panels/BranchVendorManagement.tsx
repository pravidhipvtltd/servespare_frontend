...
export const BranchVendorManagement: React.FC<{
  users: User[];
  bills: Bill[];
  onUpdate: () => void;
  ...
  const getStats = () => {
    if (viewMode === "branches") {
      ...
      const totalBranchUsers = users.filter((u) =>
      ...
      const totalMonthlyFees = vendors
        .filter((v) => v.subscriptionStatus === "active")
        .reduce((sum, v) => sum + v.monthlyFee, 0);
  ...
  const handleDeleteBranch = (branchId: string) => {
    if (!confirm("⚠️ Delete this branch? This action cannot be undone."))
      return;
  ...
  const handleDeleteVendor = (vendorId: string) => {
    if (!confirm("⚠️ Delete this vendor? This will remove all their data."))
      return;
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder={`Search ${
                viewMode === "branches" ? "branches" : "vendors"
              }...`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBranches.map((branch) => {
            const branchUsers = users.filter(
              (u) => u.workspaceId === branch.id
            );
            const branchBills = bills.filter(
              (b) => b.workspaceId === branch.id
            );
          ...
          {filteredVendors.map((vendor) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold mb-4 bg-${plan.color}-100 text-${plan.color}-700`}
                >
                  ...
                  <div className="p-3 bg-blue-50 rounded-lg">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-blue-900 font-bold text-sm">
                        {vendor.storageUsed}GB / {vendor.storageLimit}GB
                      </span>
                    </div>
                    <div className="w-full bg-blue-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${
                            (vendor.storageUsed / vendor.storageLimit) * 100
                          }%`,
                        }}
                      />
                    </div>
...
// Vendor Details Modal
const VendorDetailsModal: React.FC<{ vendor: Vendor; onClose: () => void }> = ({
  vendor,
  onClose,
}) => {
  const plan = SUBSCRIPTION_PLANS[vendor.subscriptionPlan];
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-semibold bg-${plan.color}-100 text-${plan.color}-700`}
                >
                  {plan.name}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-gray-900">
                  {new Date(vendor.subscriptionStartDate).toLocaleDateString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
...
// Subscription Manager Modal Component
const SubscriptionManagerModal: React.FC<{
  vendor: Vendor;
  onClose: () => void;
  onUpdate: () => void;
}> = ({ vendor, onClose, onUpdate }) => {
  const [selectedPlan, setSelectedPlan] = useState(vendor.subscriptionPlan);
  const [startDate, setStartDate] = useState(vendor.subscriptionStartDate);
  const [endDate, setEndDate] = useState(vendor.subscriptionEndDate);
  ...
  const handleUpdateSubscription = () => {
    ...
    auditLog.unshift({
      ...
      userId: "superadmin",
      userName: "Super Admin",
      action: "Update Vendor Subscription",
      details: `Updated subscription for vendor: ${vendor.businessName} to ${plan.name} plan`,
      category: "vendor",
    });
    saveToStorage("auditLog", auditLog);
