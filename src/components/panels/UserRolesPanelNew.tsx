...
export const UserRolesPanel: React.FC = () => {
  ...
  const handleDelete = async (userId: string) => {
    ...
    if (
      !confirm(
        `⚠️ Are you sure you want to delete ${targetUser?.name}?\n\nThis action cannot be undone!`,
      )
    ) {
      return;
    }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const filteredUsers = users.filter(
        (u: any) => !selectedUsers.includes(u.id),
      );
      localStorage.setItem("users", JSON.stringify(filteredUsers));
      setSelectedUsers([]);
            ...
            {paginatedUsers.map((user) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-12 h-12 rounded-full ${ROLE_COLORS[user.role].badge} flex items-center justify-center text-white`}
                    >
                ...
                <div
                  className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs ${ROLE_COLORS[user.role].bg} ${ROLE_COLORS[user.role].text} mb-3`}
                >
                ...
                {paginatedUsers.map((user) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full ${ROLE_COLORS[user.role].badge} flex items-center justify-center text-white text-sm`}
                        >
                          {user.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                    ...
                    <td className="p-3">
                      <div
                        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs ${ROLE_COLORS[user.role].bg} ${ROLE_COLORS[user.role].text}`}
                      >
                        <Shield className="h-3 w-3" />
                        {ROLE_LABELS[user.role]}
                      </div>
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-sm ${user.status === "active" ? "text-green-400" : "text-red-400"}`}
                        >
                          {user.status.charAt(0).toUpperCase() +
                            user.status.slice(1)}
                        </span>
