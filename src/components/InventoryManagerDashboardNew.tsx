...
import {
  Package, LogOut, Menu, X, Search, Plus, AlertTriangle, AlertCircle,
  TrendingDown, BarChart3, Box, Tag, ShoppingBag, Scan,
  FileText, Download, Upload, Filter, ChevronDown, Bell,
  Clock, Activity, TrendingUp, Boxes, PackageCheck, Bookmark,
  Grid, List, Eye, Edit2, Trash2, CheckCircle, XCircle,
  ArrowUpCircle, ArrowDownCircle, RefreshCw, Calendar, Zap,
  DollarSign, Percent, Hash, AlignLeft, Image as ImageIcon
} from 'lucide-react';
...
// Placeholder components for categories and brands
const CategoriesPanel = () => (
  <div className="space-y-6">
    ...
    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border-2 border-purple-200 p-16 text-center">
      <Grid className="w-24 h-24 text-purple-600 mx-auto mb-6" />
      <h3 className="text-2xl font-bold text-gray-900 mb-3">Categories Management</h3>
      <p className="text-gray-600 mb-6 max-w-lg mx-auto">
        This feature will allow you to create and manage product categories for better organization.
        Categories are managed through the main inventory system.
      </p>
      <button
...
const BrandsPanel = () => (
  <div className="space-y-6">
    ...
    <div className="bg-gradient-to-br from-pink-50 to-pink-100 rounded-xl border-2 border-pink-200 p-16 text-center">
      <Bookmark className="w-24 h-24 text-pink-600 mx-auto mb-6" />
      <h3 className="text-2xl font-bold text-gray-900 mb-3">Brands Management</h3>
      <p className="text-gray-600 mb-6 max-w-lg mx-auto">
        This feature will allow you to create and manage product brands and manufacturers.
        Brands are managed through the main inventory system.
      </p>
      <button
        onClick={() => window.location.reload()}
        className="px-6 py-3 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition-colors inline-flex items-center space-x-2"
      >
        <Package className="w-5 h-5" />
        <span>Go to Products</span>
      </button>
...
export const InventoryManagerDashboardNew: React.FC = () => {
  ...
  console.log('InventoryManagerDashboardNew loaded - AlertCircle imported:', typeof AlertCircle);
  ...
  const loadDashboardData = () => {
    ...
    const categories = getFromStorage('categories', [])
      .filter((c: any) => c.workspaceId === currentUser?.workspaceId);
    
    const brands = getFromStorage('brands', [])
      .filter((b: any) => b.workspaceId === currentUser?.workspaceId);
    ...
    const totalStockValue = products.reduce((sum: number, p: any) => 
      sum + ((p.currentStock || 0) * (p.purchasePrice || 0)), 0
    );
  ...
  const menuItems: MenuItem[] = [
    ...
    { id: 'purchase-orders', label: 'Purchase Orders', icon: ShoppingBag, badge: stats.pendingOrders },
    { id: 'low-stock', label: 'Low Stock Alerts', icon: AlertTriangle, badge: stats.lowStockItems },
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all cursor-pointer"
          ...
          <p className="text-blue-100 text-xs mt-2">Click to view all products</p>
        </div>

        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all cursor-pointer"
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all">
          ...
          <p className="text-4xl font-bold">रू{(stats.totalStockValue / 1000).toFixed(0)}K</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border-2 border-purple-200 p-4 hover:shadow-md transition-all cursor-pointer"
        ...
        <div className="bg-white rounded-xl border-2 border-pink-200 p-4 hover:shadow-md transition-all cursor-pointer"
        ...
        <div className="bg-white rounded-xl border-2 border-indigo-200 p-4 hover:shadow-md transition-all cursor-pointer"
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{stats.todayPurchases}</p>
            </div>
          </div>
        </div>
            ...
            {lowStockItems.length === 0 ? (
              ...
              lowStockItems.map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                  ...
                  <div className="text-right">
                    <p className="text-orange-600 font-bold">{item.currentStock} units</p>
                    <p className="text-xs text-gray-500">Min: {item.minimumStock || 10}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="p-4 space-y-2 max-h-96 overflow-y-auto">
            {recentActivity.length === 0 ? (
              ...
              recentActivity.map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-md transition-all">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {item.lastUpdated ? 'Updated' : 'Added'} {new Date(item.lastUpdated || item.createdAt).toLocaleString('en-NP')}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={`text-sm font-bold ${item.currentStock > (item.minimumStock || 10) ? 'text-green-600' : 'text-orange-600'}`}>
      ...
      <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <span className="text-sm font-semibold text-gray-900">Create PO</span>
          </button>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-gray-900">Current Stock</th>
                  <th className="px-6 py-4 text-left text-gray-900">Minimum Stock</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {lowStockItems.map((item: any) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-gray-900">{item.sku}</td>
                    <td className="px-6 py-4">
                      <span className="text-orange-600 font-bold text-lg">{item.currentStock}</span>
                    </td>
                    <td className="px-6 py-4 text-gray-900">{item.minimumStock || 10}</td>
                    <td className="px-6 py-4">
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-green-600">{stats.totalProducts - stats.outOfStock}</span>
            </div>
            <div className="flex justify-between items-center">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-gray-900">रू{stats.totalStockValue.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Avg. per Product:</span>
              <span className="font-bold text-gray-900">
                रू{stats.totalProducts > 0 ? Math.round(stats.totalStockValue / stats.totalProducts).toLocaleString() : 0}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-gray-900">{stats.todayPurchases}</span>
            </div>
            <div className="flex justify-between items-center">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 p-8 text-center">
        ...
        <p className="text-gray-600 mb-4">
          Advanced analytics, trends, and forecasting features are in development
        </p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-6">
          ...
          {activePanel === 'low-stock' && renderLowStockPanel()}
